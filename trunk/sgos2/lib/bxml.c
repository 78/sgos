...
static struct BXML_NODE* create_node( struct BXML_DATA* xml )
{
	...
	nod = (struct BXML_NODE*) kmalloc( sizeof(struct BXML_NODE) );
	if( !nod ){
		...
		case TagName:	//<tag
			...
			case ' ':	case '\r':	case '\n':	case '\t':	case '/':
				if( (int)(p-v)>0 ){	//Tag Name
					...
					if( len > 0 ){
						cur->name_len = len;
						cur->name = (char*)kmalloc( cur->name_len+1 );
						if( cur->name ){
		...
		case Text:	//<tag>Text</tag>
			if( *p == '<' ){
				if( *(p+1) == '/' ){//end tag??
					...
					if( len > 0 ){
						cur->value_len = len;
						cur->value = (char*)kmalloc( cur->value_len+1 );
						if( cur->value ){
		...
		case TagEnd:	//</tag>
			if( *p == '>' ){
				if( *v == '/' && strncmp( v+1, cur->name, MIN(cur->name_len, (int)(p-v-1)) )==0 ){
					...
					PERROR("## incorrect end tag for <%s>\n", cur->name );
				}
...
struct BXML_DATA* bxml_parse( const char* mem )
{
	...
	xml = (struct BXML_DATA*) kmalloc( sizeof(struct BXML_DATA) );
	if( !xml ){
	...
	const char* name, int create )
{
	...
	if( create ){
		...
		nod->name = (char*)kmalloc( nod->name_len+1 );
		if( nod->name ){
	...
	if( !condition ){
		...
		if( create ){
			...
			for( j=0; j<i; j++ ){
				...
				nod_attr->value = (char*)kmalloc( nod_attr->value_len+1);
...
void bxml_write( struct BXML_DATA* xml, const char* path, const void* buf, unsigned int buf_size )	
{
	...
	if( nod ){
		...
		nod->value = (char*)kmalloc( nod->value_len );
		if( nod->value )
...
void bxml_writestr( struct BXML_DATA* xml, const char* path, const char* str )	
{
	...
	if( nod ){
		...
		nod->value = (char*)kmalloc( nod->value_len+1 );
		if( nod->value )
